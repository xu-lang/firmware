...
地面端发现 lost packets 时可以在 UDP 消息中携带 `idr_code`。飞机端收到后执行：

```sh
curl localhost/request/idr
```

`alink_drone` 对 keyframe 请求做了去重和限流：

- 相同 `idr_code` 不重复处理。
- 受 `request_keyframe_interval_ms` 限制。
- 本机 TX dropped 触发的 keyframe 请求也受同一类间隔限制。
